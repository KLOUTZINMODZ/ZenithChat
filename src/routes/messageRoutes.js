...
// Endpoint para sincronização de mensagens offline
router.post('/sync', auth, async (req, res) => {
  try {
    const { conversationId, message } = req.body;
    const userId = req.user._id || req.user.id;
    
    logger.info(`[OFFLINE SYNC] Sincronizando mensagem offline para conversa ${conversationId} por usuário ${userId}`);
    
    // Verificar acesso à conversa
    const conversation = await Conversation.findById(conversationId);
    if (!conversation || !conversation.participants.includes(userId)) {
      logger.warn(`[OFFLINE SYNC] Acesso negado para conversa ${conversationId} - usuário ${userId}`);
    ...
    const existingMessage = await Message.findOne({
      ...
      createdAt: {
        $gte: new Date(message.timestamp - 5000), // 5 segundos de tolerância
        $lte: new Date(message.timestamp + 5000)
      }
    });

    if (existingMessage) {
      logger.info(`[OFFLINE SYNC] Mensagem duplicada detectada, retornando existente: ${existingMessage._id}`);
      return res.json({
        success: true,
        message: {
          ...existingMessage.toObject(),
          content: decryptMessage(existingMessage.content)
        },
        isDuplicate: true
      });
    }
    ...
    // Emitir via WebSocket para outros participantes
    ...
    logger.info(`[OFFLINE SYNC] Mensagem sincronizada com sucesso: ${newMessage._id}`);
    ...
    logger.error('[OFFLINE SYNC] Erro ao sincronizar mensagem offline:', error);
    res.status(500).json({ 
      success: false,
